...
class LoggingMarker(StreamMonitorABC):
    ...
    def __mark_all_in_logger(self, logger, msg, args, exc_info=None, extra=None):
        ...
        record = logger.makeRecord(logger.name, INFO, fn, lno, msg, args, exc_info, func, extra)
        for handler in handlers:
            handler.handle(record)

    def __mark_all_loggers(self, bracket, fmat, *args, **kwargs):
        rep_cnt = int(70/len(bracket))
        for logger in self.__loggers.values():
            self.__mark_all_in_logger(logger, bracket * rep_cnt, [])
            self.__mark_all_in_logger(logger, fmat, args, **kwargs)
            self.__mark_all_in_logger(logger, bracket * rep_cnt, [])
